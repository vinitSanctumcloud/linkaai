...
import { Copy, Code, ExternalLink, Globe, Smartphone, Bot, Eye } from 'lucide-react'
...
export default function EmbedPage() {
  ...
  const chatUrl = settings?.customUrl ? `${baseUrl}/chat/${settings.customUrl}` : ''
    ...
    <DashboardLayout>
      <div className="space-y-6 md:space-y-8 sm:px-6 lg:px-8 py-6">
        ...
        <div className="text-center md:text-left">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Embed Your AI Agent</h1>
          <p className="text-gray-600 mt-2 text-sm sm:text-base">
            Get the code to add your AI chat agent to any website
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
          ...
          <div className="space-y-4 md:space-y-6">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="width" className="text-sm sm:text-base">Width (px)</Label>
                    <Input
                      id="width"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="height" className="text-sm sm:text-base">Height (px)</Label>
                    <Input
                      id="height"
                ...
                {chatUrl && (
                  <div className="pt-4">
                    <Label className="text-sm sm:text-base">Your Chat URL</Label>
            ...
            <Card className="border border-gray-200 shadow-sm rounded-xl">
              <CardHeader>
                <CardTitle className="text-lg sm:text-xl">Embed Options</CardTitle>
                <CardDescription className="text-sm sm:text-base">
              ...
              <CardContent>
                <Tabs defaultValue="iframe" className="w-full">
                  <TabsList className="grid w-full grid-cols-3 h-auto p-1">
                    <TabsTrigger value="iframe" className="py-2 text-xs sm:text-sm">
                      <Globe className="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2" />
                      Iframe
                    </TabsTrigger>
                    <TabsTrigger value="widget" className="py-2 text-xs sm:text-sm">
                      <Code className="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2" />
                      Widget
                    </TabsTrigger>
                    <TabsTrigger value="popup" className="py-2 text-xs sm:text-sm">
                  ...
                  <TabsContent value="popup" className="mt-4">
                    <div className="space-y-3">
                      <div>
                        ...
                        <p className="text-gray-600 text-xs sm:text-sm mb-3">
                          Floating chat button that opens your AI agent in a popup window.
                        </p>
                      </div>
                      <div className="relative">
                        <Textarea
              ...
              <CardContent className="space-y-3">
                {[
                  ...
                  <div key={index} className="flex items-start">
                    <Badge variant="secondary" className="mr-3 mt-0.5 bg-orange-100 text-orange-800">
                      {index + 1}
                    </Badge>
              ...
              <CardContent className="px-3 sm:px-6 pb-3 sm:pb-6">
                <div className="p-2 sm:p-6 rounded-lg">
                  ...
                  <div className="mt-4 sm:mt-6 flex flex-col sm:flex-row justify-center gap-2 sm:gap-3">
                    <Button
                      variant={embedSize.width === '300' ? 'default' : 'outline'}
                      size="sm"
                      onClick={() => setEmbedSize({ width: '300', height: '500' })}
                      className="text-xs sm:text-sm py-1 sm:py-2"
                    >
                      Mobile
                    </Button>
                    <Button
                      variant={embedSize.width === '650' ? 'default' : 'outline'}
                      size="sm"
                      onClick={() => setEmbedSize({ width: '650', height: '600' })}
                      className="text-xs sm:text-sm py-1 sm:py-2"
                    >
                      Desktop
                    </Button>
